...
impl OrchestratorConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (k, p) in &self.providers {
            if p.name != *k {
                return Err(format!(
                    "provider key '{}' does not match provider.name '{}'",
                    k, p.name
                ));
            }
...
mod tests {
    ...
    async fn test_fan_out_execute_selects_available_providers() {
        ...
        assert!(
            res.is_ok(),
            "expected fan_out_execute to be implemented and return Ok"
        );
    ...
    async fn test_load_orchestrator_state_from_view() {
        ...
        client
            .create_view("orchestrator", "by_provider", map_function)
            .await
        ...
        let view_results = client
            .query_view("orchestrator", "by_provider")
            .await
        ...
        let res =
            OrchestratorState::load_from_couch_view(&client, "orchestrator", "by_provider").await;
        assert!(
            res.is_ok(),
            "expected load_from_couch_view to be implemented and return Ok: {:?}",
            res.err()
        );
        ...
        if let Ok(loaded_state) = res {
            ...
            for (provider_name, original_provider) in &state.providers {
                assert!(
                    loaded_state.providers.contains_key(provider_name),
                    "Loaded state missing provider: {}",
                    provider_name
                );
    ...
    async fn test_save_and_load_orchestrator_state_as_single_doc() {
        ...
        assert!(
            save_result.is_ok(),
            "Expected save_to_couchdb to be implemented and return Ok - TDD Red test"
        );
    ...
    async fn test_fan_out_respects_max_fanout_limit() {
        ...
        if let Ok(result_str) = res {
            ...
            assert_eq!(
                ...
                2,
                "Expected 2 providers (max_fanout), got {}",
                selected_providers.len()
            );
        }
    }
    ...
    async fn test_fan_out_excludes_providers_at_zero_quota() {
        ...
        if let Ok(result_str) = res {
            ...
            assert_eq!(
                selected_providers.len(),
                1,
                "Expected 1 provider with valid quota, got {}",
                selected_providers.len()
            );
            assert!(
                selected_providers.contains(&"working_provider".to_string()),
                "Should have selected the working provider"
            );

            // Should NOT have selected any of the zero-quota providers
            assert!(
                !selected_providers.contains(&"zero_quota_provider_rpm".to_string()),
                "Should NOT have selected provider with zero RPM"
            );
            assert!(
                !selected_providers.contains(&"zero_quota_provider_tpm".to_string()),
                "Should NOT have selected provider with zero TPM"
            );
            assert!(
                !selected_providers.contains(&"zero_quota_provider_both".to_string()),
