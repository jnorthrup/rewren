...
export async function loadCliConfig(
  ...
  // IDE mode: ON by default, opt-out only
  // Behavior priority (highest -> lowest):
  // 1) explicit CLI flag (--ide-mode / --no-ide-mode)
  // 2) settings.ideMode if present
  // 3) NO_IDE_MODE env var to disable
  // 4) default: ON (always enabled, opt-out only)
  const cliExplicit = typeof argv.ideMode === 'boolean' ? argv.ideMode : undefined;
  const settingsExplicit = typeof settings.ideMode === 'boolean' ? settings.ideMode : undefined;
  const noIdeModeEnv = process.env.NO_IDE_MODE === '1' || process.env.NO_IDE_MODE === 'true';

  let ideMode: boolean;
  if (cliExplicit !== undefined) {
    // CLI explicit true/false wins
    ideMode = cliExplicit;
  } else if (settingsExplicit !== undefined) {
    // Settings explicit true/false next
    ideMode = settingsExplicit;
  } else if (noIdeModeEnv) {
    // NO_IDE_MODE env var disables
    ideMode = false;
  } else {
    // Default: always ON (opt-out only via NO_IDE_MODE, --no-ide-mode, or settings)
    ideMode = true;
  }

  // Always disable if running in sandbox
  if (process.env.SANDBOX) ideMode = false;
