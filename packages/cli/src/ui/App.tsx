...
const App = ({ config, settings, startupWarnings = [], version }: AppProps) => {
  ...
  // Initialize provider tree for dual-pane UI (imported dynamically to avoid circular deps)
  const [providerTree, setProviderTree] = useState<any>(null);
  const [providerTreeInitialized, setProviderTreeInitialized] = useState(false);

  useEffect(() => {
    import('@wren-coder/wren-coder-cli-core').then(({ ProviderTreeRoot }) => {
      const tree = new ProviderTreeRoot();
      tree.initialize().then(() => {
        setProviderTree(tree);
        setProviderTreeInitialized(true);
      });
    });
  }, []);
  ...
  // Helper function to format model names with provider prefix
  const formatModelWithProvider = useCallback((modelName: string) => {
    const modelConfig = getModelConfig(modelName);
    const providerPrefix = modelConfig ? `[${modelConfig.provider}] ` : '';
  ...
  useEffect(() => {
    const flashFallbackHandler = async (
      ...
      if (error && isProQuotaExceededError(error)) {
        if (isPaidTier) {
          message = `⚡ You have reached your daily ${formatModelWithProvider(currentModel)} quota limit.
⚡ Automatically switching from ${formatModelWithProvider(currentModel)} to ${formatModelWithProvider(fallbackModel)} for the remainder of this session.
⚡ To continue accessing the ${formatModelWithProvider(currentModel)} model today, consider using /auth to switch to using a paid API key from AI Studio at https://aistudio.google.com/apikey`;
        } else {
...
⚡ To continue accessing the ${currentModel} model today, consider using /auth to switch to using a paid API key from AI Studio at https://aistudio.google.com/apikey`;
          ...
          // Default fallback message for other cases (like consecutive 429s)
          message = `⚡ Automatically switching from ${formatModelWithProvider(currentModel)} to ${formatModelWithProvider(fallbackModel)} for faster responses for the remainder of this session.
⚡ Possible reasons for this are that you have received multiple consecutive capacity errors or you have reached your daily ${formatModelWithProvider(currentModel)} quota limit
⚡ To increase your limits, upgrade to a Gemini Code Assist Standard or Enterprise plan with higher limits at https://goo.gle/set-up-gemini-code-assist
⚡ Or you can utilize a Gemini API Key. See: https://goo.gle/gemini-cli-docs-auth#gemini-api-key
⚡ You can switch authentication methods by typing /auth`;
        }
  ...
  }, [config, addItem, userTier, formatModelWithProvider]);
