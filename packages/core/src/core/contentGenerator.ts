...
export async function createContentGeneratorConfig(
  ...
  // Z.AI / GLM provider environment variables (official and legacy names)
  ...
  if (
    ...
    maybeLogContentGeneratorConfig(contentGeneratorConfig);
  ...
  if (authType === AuthType.USE_OPENAI_COMPATIBLE || !authType) {
...
// Diagnostic helper: log resolved provider configuration without revealing secrets.
...
function maybeLogContentGeneratorConfig(cfg: ContentGeneratorConfig) {
  if (process.env.WREN_LOG_CONTENT_GENERATOR_CONFIG !== '1') return;
  try {
    console.info('[WREN DEBUG] createContentGeneratorConfig ->', {
      model: cfg.model,
      authType: cfg.authType,
      apiKeyPresent: !!cfg.apiKey,
      apiKeyMasked: maskKey(cfg.apiKey),
      baseURL: cfg.baseURL || '<default>',
      vertexai: cfg.vertexai || false,
    });
  } catch (_e) {
