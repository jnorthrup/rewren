...
export class OpenAIContentGenerator implements ContentGenerator {
  ...
  // Read model dynamically from config to support runtime model changes
  ...
  constructor(apiKey: string, _model: string, config: Config) {
    ...
    // Get base URL from config first, fallback to env var
    const contentGeneratorConfig = this.config.getContentGeneratorConfig();
    this.baseURL = contentGeneratorConfig?.baseURL || process.env.OPENAI_BASE_URL || '';
  ...
  /**
   * Check if this is an NVIDIA GPT-OSS model that uses the special responses API
   */
  ...
  private isZAIModel(): boolean {
    const provider = this.getProviderFromBaseURL();
    return provider === Providers.ZAI;
  }

  /**
   * Convert Gemini messages to GPT-OSS input format (array of strings)
   */
  private convertMessagesToGPTOSSInput(request: GenerateContentParameters): string[] {
    const input: string[] = [];
    const contents = Array.isArray(request.contents) ? request.contents : [request.contents];
    for (const content of contents) {
      // Check if this is a Content object (not a Part)
  ...
  private getProviderFromBaseURL(): Providers | null {
    ...
    } else if (this.baseURL.includes('aliyuncs.com')) {
      return Providers.QWEN;
    } else if (this.baseURL.includes('moonshot')) {
      return Providers.MOONSHOT_AI;
    } else if (this.baseURL.includes('huggingface')) {
      return Providers.HUGGINGFACE;
    } else if (this.baseURL.includes('x.ai')) {
      return Providers.XAI;
    } else if (this.baseURL.includes('z.ai')) {
      return Providers.ZAI;
    } else if (this.baseURL.includes('oai.endpoints.kepler.ai.cloud.ovh.net')) {
      return Providers.KILO;
    } else if (this.baseURL.includes('generativelanguage.googleapis.com')) {
      return Providers.GOOGLE; // For Gemini
    }
    
    // Default to OPENAI for generic OpenAI-compatible services
    return Providers.OPENAI;
  }
  ...
  /**
   * Find a fallback model that supports function calling
   */
  private async findFallbackModelForTools(): Promise<string | null> {
    const { listModels } = await import('../config/modelRegistry.js');
    const allModels = listModels();
    
    // Get current provider
    const currentProvider = this.getProviderFromBaseURL();
    
    // First try to find another model from the same provider that supports tools
    for (const modelConfig of allModels) {
      if (modelConfig.provider === currentProvider && 
          modelConfig.capabilities?.functionCalling && 
          modelConfig.name !== this.model) {
        return modelConfig.name;
      }
    }
    
    // If no model from same provider, try any provider that supports tools
    for (const modelConfig of allModels) {
      if (modelConfig.capabilities?.functionCalling && modelConfig.name !== this.model) {
        return modelConfig.name;
      }
    }
  ...
  async generateContent(
    ...
    try {
      ...
      if (request.config?.tools && request.config.tools.length > 0) {
        const { getModelConfig } = await import('../config/modelRegistry.js');
        const modelConfig = getModelConfig(this.model);
        if (!modelConfig?.capabilities?.functionCalling) {
          // Try to switch to a fallback model that supports function calling
          const fallbackModel = await this.findFallbackModelForTools();
          if (fallbackModel) {
            console.warn(`Model ${this.model} does not support tool calling. Switching to ${fallbackModel} for this request.`);
            this.config.setModel(fallbackModel);
            // Re-initialize with new model
            const { createContentGenerator } = await import('./contentGenerator.js');
            const newGenerator = await createContentGenerator(
              { ...this.config.getContentGeneratorConfig(), model: fallbackModel },
              this.config,
              undefined
            );
            return newGenerator.generateContent(request);
          }
          throw new Error(`Model ${this.model} does not support tool calling. Please select a different model that supports function calling.`);
        }
      }

      // Check if this is a GPT-OSS model that needs special handling
      if (this.isGPTOSSModel()) {
        // Check if tools are requested and model supports function calling
        if (request.config?.tools && request.config.tools.length > 0) {
          const { getModelConfig } = await import('../config/modelRegistry.js');
          const modelConfig = getModelConfig(this.model);
          if (!modelConfig?.capabilities?.functionCalling) {
          ...
          // If model claims to support function calling but uses responses API, we can't use tools
          throw new Error(`GPT-OSS models (${this.model}) do not support tool calling via the responses API. Please select a different model that supports function calling.`);
        }
        
        const input = this.convertMessagesToGPTOSSInput(request);
        const samplingParams = this.buildSamplingParameters(request);

        // Get model config for reasoning parameters
        const { getModelConfig } = await import('../config/modelRegistry.js');
        const modelConfig = getModelConfig(this.model);

        // Build request with OpenRouter Responses API parameters
        const gptOssRequest: any = {
          model: this.model,
          input,
          max_output_tokens: samplingParams.max_tokens || 8192,
          temperature: samplingParams.temperature || 1,
          top_p: samplingParams.top_p || 1,
        };

        // Add reasoning object if configured (OpenRouter Responses API format)
        if (modelConfig?.reasoning) {
          gptOssRequest.reasoning = {};
          if (modelConfig.reasoning.effort) {
            gptOssRequest.reasoning.effort = modelConfig.reasoning.effort;
          }
          if (modelConfig.reasoning.max_tokens) {
            gptOssRequest.reasoning.max_tokens = modelConfig.reasoning.max_tokens;
          }
          if (modelConfig.reasoning.exclude !== undefined) {
            gptOssRequest.reasoning.exclude = modelConfig.reasoning.exclude;
          }
        }

        // Add verbosity if configured
        if (modelConfig?.verbosity) {
          gptOssRequest.verbosity = modelConfig.verbosity;
        }

        // Add include_reasoning flag to get reasoning in response
        if (modelConfig?.includeReasoning) {
          gptOssRequest.include_reasoning = true;
        }

        // Use responses.create for GPT-OSS models
        const response = await (this.client as any).responses.create(gptOssRequest);

        // Log full COT response for trustless verification (non-streaming)
        if (process.env.WREN_COT_LOG_FILE || process.env.WREN_DEBUG_COT === 'true') {
          const logEntry = {
            timestamp: new Date().toISOString(),
            type: 'non-streaming-response',
            model: this.model,
            output: response.output || '',
            outputLength: (response.output || '').length,
          };
          console.error(`[COT-FULL] ${JSON.stringify(logEntry)}`);
        }
        ...
        if (provider) {
          ProviderConfigService.updatePerformanceStats(provider, true, durationMs);
          // Record in new metrics system
          MetricsIntegration.getInstance().recordSuccess(provider, this.model, durationMs);
        }
        return geminiResponse;
      }

      // Standard OpenAI chat completions flow
      ...
      if (provider) {
        ...
        if (response.usageMetadata?.candidatesTokenCount && durationMs > 0) {
          tokensPerSecond = (response.usageMetadata.candidatesTokenCount / durationMs) * 1000;
        }
        
        ProviderConfigService.updatePerformanceStats(provider, true, durationMs, tokensPerSecond);
      }

      // Persist model usage metrics (timestamping) for resume/inspection
      try {
        ModelUsageService.recordUsage(this.model, true);
      } catch (_e) {
        // best-effort
      }
      ...
      if (provider) {
        ProviderConfigService.updatePerformanceStats(provider, false, durationMs);
        // Record in new metrics system
        MetricsIntegration.getInstance().recordError(provider, this.model, errorMessage);
      }

      try { ModelUsageService.recordUsage(this.model, false); } catch (_e) { /* best-effort */ }
  ...
  async generateContentStream(
    ...
    try {
      ...
      if (request.config?.tools && request.config.tools.length > 0) {
        ...
        if (!modelConfig?.capabilities?.functionCalling) {
          ...
          if (fallbackModel) {
            ...
            return newGenerator.generateContentStream(request);
          }
          throw new Error(`Model ${this.model} does not support tool calling. Please select a different model that supports function calling.`);
        }
      }
      ...
      if (this.isGPTOSSModel()) {
        ...
        // Use responses.create with streaming for GPT-OSS models
        const stream = await (this.client as any).responses.create(gptOssRequest);

        // Yield streaming chunks in Gemini format with COT metadata
        const modelName = this.model; // Capture for async generator
        return (async function* () {
          // Independent channel state tracking (not chained)
          let reasoningText = '';
          let commentaryText = '';
          let outputText = '';
          let reasoningStarted = false;
          let commentaryStarted = false;
          const cotLogger = process.env.WREN_COT_LOG_FILE;

          for await (const chunk of stream) {
            const response = new GenerateContentResponse();

            // ANALYSIS CHANNEL: Independent rendering with 👽 markers
            if (chunk.type === 'response.reasoning_text.delta') {
              // Add alien emoji at the start of reasoning (analysis channel)
              if (!reasoningStarted) {
                reasoningStarted = true;
                const startMarker = new GenerateContentResponse();
                // @ts-expect-error - adding custom isReasoning property
                const markerPart: Part = { text: '👽 ', isReasoning: true, isChannelMarker: true };
                startMarker.candidates = [{
                  content: {
                    parts: [markerPart],
                    role: 'model' as const,
                  },
              ...
              // Log COT for trustless software verification
              if (cotLogger || process.env.WREN_DEBUG_COT === 'true') {
                const logEntry = {
                  timestamp: new Date().toISOString(),
                  type: 'reasoning',
                  content: chunk.delta,
                ...
                // Console log with special formatting
                console.error(`[COT-REASONING] ${JSON.stringify(logEntry)}`);
              }

              // Mark this chunk as reasoning with custom metadata
              // @ts-expect-error - adding custom isReasoning property for UI coloring
              const reasoningPart: Part = { text: chunk.delta, isReasoning: true };
              response.candidates = [{
                content: {
                  parts: [reasoningPart],
                  role: 'model' as const,
            ...
            // COMMENTARY CHANNEL: Independent rendering with 👓 markers
            } else if (chunk.type === 'response.commentary.delta') {
              // Add glasses emoji at the start of commentary channel
              if (!commentaryStarted) {
                commentaryStarted = true;
                const startMarker = new GenerateContentResponse();
                // @ts-expect-error - adding custom isCommentary property
                const markerPart: Part = { text: '👓 ', isCommentary: true, isChannelMarker: true };
                startMarker.candidates = [{
                  content: {
                    parts: [markerPart],
                    role: 'model' as const,
              ...
              commentaryText += chunk.delta;

              // Log commentary for verification
              if (cotLogger || process.env.WREN_DEBUG_COT === 'true') {
                const logEntry = {
                  timestamp: new Date().toISOString(),
                  type: 'commentary',
                ...
                console.error(`[COT-COMMENTARY] ${JSON.stringify(logEntry)}`);
              }

              // Mark this chunk as commentary with custom metadata
              // @ts-expect-error - adding custom isCommentary property for UI coloring
              const commentaryPart: Part = { text: chunk.delta, isCommentary: true };
              response.candidates = [{
                content: {
                  parts: [commentaryPart],
                  role: 'model' as const,
                },
                finishReason: FinishReason.FINISH_REASON_UNSPECIFIED,
              }];
            ...
            // FINAL CHANNEL: Independent rendering with no markers
            } else if (chunk.type === 'response.output_text.delta') {
              // End any active channels before starting final output
              // (each channel closes independently, not chained)
              if (reasoningStarted && reasoningText.length > 0) {
                const endMarker = new GenerateContentResponse();
                // @ts-expect-error - adding custom isReasoning property
                const markerPart: Part = { text: ' 👽', isReasoning: true, isChannelMarker: true };
                endMarker.candidates = [{
                  content: {
                    parts: [markerPart],
                    role: 'model' as const,
                  },
              ...
              if (commentaryStarted && commentaryText.length > 0) {
                const endMarker = new GenerateContentResponse();
                // @ts-expect-error - adding custom isCommentary property
                const markerPart: Part = { text: ' 👓', isCommentary: true, isChannelMarker: true };
                endMarker.candidates = [{
              ...
              if (cotLogger || process.env.WREN_DEBUG_COT === 'true') {
                ...
                console.error(`[COT-FINAL] ${JSON.stringify(logEntry)}`);
              }

              // Final channel: no special properties, just plain text
              response.candidates = [{
                content: {
              ...
              // Log completion summary for trustless verification (all channels)
              if (cotLogger || process.env.WREN_DEBUG_COT === 'true') {
                const logEntry = {
                  timestamp: new Date().toISOString(),
                  type: 'done',
                  model: modelName,
                  channels: {
                    analysis: reasoningText.length,
                    commentary: commentaryText.length,
                    final: outputText.length,
                  },
                };
                console.error(`[COT-DONE] ${JSON.stringify(logEntry)}`);
              }
      ...
      // Standard OpenAI chat completions streaming flow
      const messages = this.convertToOpenAIFormat(request);
      // Build sampling parameters with clear priority
      const samplingParams = this.buildSamplingParameters(request);
      ...
      const wrappedGenerator = async function* (this: OpenAIContentGenerator) {
        try {
          ...
          if (provider) {
            let tokensPerSecond = 0;
            if (finalUsageMetadata?.candidatesTokenCount && durationMs > 0) {
              tokensPerSecond = (finalUsageMetadata.candidatesTokenCount / durationMs) * 1000;
            }
            
            ProviderConfigService.updatePerformanceStats(provider, true, durationMs, tokensPerSecond);
          }
          // Persist model usage timestamp
          try { ModelUsageService.recordUsage(this.model, true); } catch (_e) { /* best-effort, ignore */ }
        } catch (error) {
          const durationMs = Date.now() - startTime;
          ...
          try { ModelUsageService.recordUsage(this.model, false); } catch (_e) { /* best-effort, ignore */ }
  ...
  async countTokens(
    ...
    try {
      ...
      // Update performance stats for successful request (duration of the estimation)
      if (provider) {
        const durationMs = Date.now() - startTime;
        ProviderConfigService.updatePerformanceStats(provider, true, durationMs);
      }
      ...
      if (choice.finish_reason) {
        for (const [, accumulatedCall] of this.streamingToolCalls) {
          if (accumulatedCall.name) {
            ...
            // Generate tool_call_id if VLLM parser doesn't provide one
            const toolCallId = accumulatedCall.id || this.generateToolCallId();
  ...
  /**
   * Generate a unique tool call ID for VLLM parsers that don't provide one
   */
  private generateToolCallId(): string {
    return `call_${++this.toolCallIdCounter}`;
  }
